...
const HomePage = () => {
  ...
  const handleDelete = async (record) => {
    try {
      setLoading(true);
      await axios.post("https://backend-zeth.onrender.com/api/v1/transections/delete-transection", {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editable) {
        ...
        await axios.post("https://backend-zeth.onrender.com/api/v1/transections/add-transection", {
          ...values,
          userid: user._id,
        });
        setLoading(false);
